...
int Player::age() const
{
    ...
    return m_age;  // DONE trivial
}

int Player::health() const
{

    return m_health;  //DONE trivial
}
...
int City::rows() const
{
    return m_rows;  // DONE Trivial
}

int City::cols() const
{
    return m_cols;  // DONE Trivial
}
...
void City::display() const
{
    ...
    for(int i = 0; i < m_nTooters; i++){
        int numOToot = nTootersAt(m_tooters[i]->row(), m_tooters[i]->col());
        char& tChar = grid[m_tooters[i]->row()-1][m_tooters[i]->col()-1];
        if (numOToot == 1)
            tChar = 'T';
        else if (numOToot > 1 && numOToot < 9)
        {
            switch (numOToot){
                case 2:
                    tChar = '2';
...
void City::preachToTootersAroundPlayer()
{
    ...
    while (i < m_nTooters){
        ...
        bool plsdelete  = false;
        if(m_tooters[i] -> row() == m_player -> row()-1 && m_tooters[i] -> col() == m_player -> col()) // checks above player
            plsdelete = true;
        else if(m_tooters[i] -> row() == m_player -> row()+1 && m_tooters[i] -> col() == m_player -> col()) // under player
            plsdelete = true;
        else if(m_tooters[i] -> row() == m_player -> row() && m_tooters[i] -> col() == m_player -> col() -1) //left of player
            plsdelete = true;
        else if(m_tooters[i] -> row() == m_player -> row() && m_tooters[i] -> col() == m_player -> col()+1) //right of player
            plsdelete = true;
        else if(m_tooters[i] -> row() == m_player -> row()-1 && m_tooters[i] -> col() == m_player -> col()-1) // upper left
            plsdelete = true;
        else if(m_tooters[i] -> row() == m_player -> row()+1 && m_tooters[i] -> col() == m_player -> col()+1) //lower right
            plsdelete = true;
        else if(m_tooters[i] -> row() == m_player -> row()-1 && m_tooters[i] -> col() == m_player -> col()+1) //upper right
            plsdelete = true;
        else if(m_tooters[i] -> row() == m_player -> row()+1 && m_tooters[i] -> col() == m_player -> col()-1) //lower left
            plsdelete = true;
        
        if (plsdelete){
            delete m_tooters[i];
            for (int k = i; k < m_nTooters; k++){
    ...
    //deletes tooter
    //shift the pointers
    //shift i back one after deleting a tooter and shifting pointers
...
void City::moveTooters()
{
    ...
    {
        m_tooters[k]->move();
        if(m_player->row() == m_tooters[k]->row() - 1 && m_player->col() == m_tooters[k]->col()) //if player is above toot and same col
        {
            m_player->getGassed();
        }
        else if (m_player->row() == m_tooters[k]->row() + 1 && m_player->col() == m_tooters[k]->col()) //if player is below toot and same col
        {
            m_player->getGassed();
        }
        else if (m_player->row() == m_tooters[k]->row() && m_player->col() == m_tooters[k]->col() - 1) //if player is left of toot and same row
        {
            m_player->getGassed();
        }
        else if (m_player->row() == m_tooters[k]->row() && m_player->col() == m_tooters[k]->col() + 1) //if player is right of toot and same row
        {
            m_player->getGassed();
        }
    }
